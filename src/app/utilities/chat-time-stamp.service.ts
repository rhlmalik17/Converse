...
class ChatTimeStampService {
  ...
  sortConversationListUsingTimeStamp(
    allConversations: ConversationType
  ): Array<string> {
    if (!allConversations || Object.keys(allConversations).length < 1)
      return [];

    return Object.keys(allConversations).sort(
      (conversationOne: string, conversationTwo: string) => {
        let timeOne: Date = allConversations[conversationOne]
          ?.updated_at as Date;
        let timeTwo: Date = allConversations[conversationTwo]
          ?.updated_at as Date;
  ...
  showSegregator(messages: Array<Message>, index: number): string {
    if (
      !messages ||
      index === undefined ||
      index < 1 ||
      ...
      return "";
    return !this.sameDay(
      new Date(messages[index - 1].updated_at),
      new Date(messages[index].updated_at)
    )
      ? this.sameDay(new Date(), new Date(messages[index].updated_at))
        ? " Today"
        : this.getChatMessageTimeStamp(messages[index].updated_at)
      : "";
  }
  ...
  invalidTimeStamp(messages: Array<Message>, index: number): boolean {
    ...
    return (
      this.getChatMessageTimeStamp(previousMessage.updated_at) ===
        this.getChatMessageTimeStamp(currentMessage.updated_at) &&
